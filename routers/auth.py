...
async def get_current_user(token: str = Depends(oath2_bearer), db: session = Depends(get_db)):
    try:
        ...
        score: int = user.current_score
        role: str = user.role
        games_played: str = user.games_played
        last_asked_question: int = user.last_asked_question
        if username is None or user_id is None or role is None:
            raise get_user_exception()
        return {"username": username, "id": user_id, "role": role, "score": score, "games_played": games_played, "last_asked_question": last_asked_question}
    except JWTError:
        raise get_user_exception()
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    user: models.Users = authenticate_user(form_data.username, form_data.password, db)
    if not user:
